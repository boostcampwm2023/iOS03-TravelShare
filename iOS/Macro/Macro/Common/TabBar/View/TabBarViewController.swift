...
final class TabbarViewController: UIViewController {
    ...
    override func viewDidLoad() {
        setUpLayout()
        bind()
        inactiveTabBarCenterView.isHidden = false
        tabBarBackgroundLargeCirclceView.isHidden = true
...
private extension TabbarViewController {
    ...
    @objc func inactiveCenterHandleTapGesture(_ sender: UITapGestureRecognizer) {
        guard viewModel.timer == nil else { return }
        inactiveTabBarCenterView.isHidden = true
        tabBarBackgroundLargeCirclceView.isHidden = false
        appearEvent()
    }
    ...
    func disAppearEvent() {
        ...
        viewModel.timer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
            if self.viewModel.animationStep < 2 {
                ...
                self.viewModel.animationStep = 0
                self.stopTimer()
                self.inactiveTabBarCenterView.isHidden = false
                self.tabBarBackgroundLargeCirclceView.isHidden = true
            }
        }
    }
